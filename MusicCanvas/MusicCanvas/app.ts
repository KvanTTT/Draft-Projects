...
declare var teoria: any;
var musicCanvas: MusicCanvas;

class OscillatorGain {
    oscillator: OscillatorNode;
    gainNode: GainNode;

    constructor(oscillator: OscillatorNode, gainNode: GainNode) {
...
class MusicItem {
    ...
    constructor(volume: number, timbre: number) {
        this.volume = volume;
        this.timbre = timbre;
    }
}
...
class MusicCanvas {
    ...
    sideColorB: number;
    sideWidth: number;
    defaultMsPerNote: number;
    defaultNotesCount: number;
    defaultTonesCount: number;
    defaultVolume: number;
    baseOctave: number;
    baseNote: number;

    audioContext: AudioContext;
    oscillatorGains: OscillatorGain[];
    compressor: DynamicsCompressorNode;

    tmpOscGains: OscillatorGain[];
    tempOscCount: number;
    curTempOsc: number;
    curVolume: number;
            ...
            for (var j = 0; j < this.notesCount; j++) {
                ...
                for (var k = startIndY; k < endIndY; k++) {
                    for (var l = startIndX; l < endIndX; l++) {
                        ...
                        if (this.data[index + 0] != this.bckColorR &&
                            this.data[index + 1] != this.bckColorG &&
                            this.data[index + 2] != this.bckColorB)
                            filledCount++;
                    }
                }
    ...
    initOscillators(count: number): Array<OscillatorGain> {
        var result = new Array<OscillatorGain>();
    ...
    recalculateFrequences() {
        ...
        var scale: string[] = note.scale(scaleStr).simple();
        ...
        for (var i = 0; i < this.tonesCount; i++) {
            ...
            this.oscillatorGains[this.tonesCount - 1 - i].oscillator.type = <OscillatorType><any>tone;
        }
    }

    noteToInd(note: string): number {
        var ind = note.charCodeAt(0) - 'a'.charCodeAt(0) + 7;
        return (ind - 2) % 7;
    }

    startTone(oscillatorGain: OscillatorGain) {
    ...
    stopTone(oscillatorGain: OscillatorGain) {
        var now = this.audioContext.currentTime;
        oscillatorGain.gainNode.gain.cancelScheduledValues(now);
        oscillatorGain.gainNode.gain.setValueAtTime(oscillatorGain.gainNode.gain.value, now);
    ...
    playTone(oscillatorGain: OscillatorGain, volume: number) {
    ...
    clearCells() {
        this.cells = new Array < Array < MusicItem>>();
        for (var i = 0; i < this.tonesCount; i++) {
            this.cells[i] = [];
            for (var j = 0; j < this.notesCount; j++) {
                this.cells[i][j] = new MusicItem(0, 0);
...
$(function () {
    ...
    (<any>$("#scale")).selectmenu({
        change: (event: Event, ui) => {
            musicCanvas.recalculateFrequences();
        }
    });

    (<any>$("#startNode")).selectmenu({
        change: (event: Event, ui) => {
            musicCanvas.recalculateFrequences();
        }
    });

    (<any>$("#startOctave")).selectmenu({
        change: (event: Event, ui) => {
            musicCanvas.recalculateFrequences();
        }
    });
